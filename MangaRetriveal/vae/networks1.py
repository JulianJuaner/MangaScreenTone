...
class GaborVAE(nn.Module):
    def __init__(self):
        ...
        self.enc = nn.Sequential(
            ...
            nn.InstanceNorm2d(64),
            nn.ReLU(),
            nn.Conv2d(64,CHANNEL*2,kernel_size=1,stride=1,padding=0),
            nn.InstanceNorm2d(CHANNEL*2),
            nn.Tanh())
        self.dec = nn.Sequential(
            nn.Conv2d(CHANNEL,64,kernel_size=1,stride=1,padding=0),
            nn.InstanceNorm2d(64),
            nn.ReLU(),
            nn.Conv2d(64,64,kernel_size=1,stride=1,padding=0),
            nn.InstanceNorm2d(64),
    ...
    def forward(self, x):
        inter = self.enc(x)
        if QUANTI:
            interout = torch.clamp((inter[:,:CHANNEL,:,:].data+1)/2.0, 0,1)
            interout = ((interout*255).round()/255.0)*2 - 1
