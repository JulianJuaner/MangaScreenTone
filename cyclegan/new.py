...
parser.add_argument("--datasize", type=int, default=3000, help="size of a dataset")
...
class CYCLEGAN:
    def __init__(self, opt):
        ...
        self.loss_names = ['GAN_AB', 'GAN_BA', 'cycle_A', 'cycle_B', 'id_A', 'id_B',\
                    'mask_A', 'mask_B', 'D_A', 'D_B', 'perc_A', 'perc_B','G','D','cycle','mask_','identity']
    ...
    def sample_images(self, batches_done, opt):
        ...
        save_image(image_grid, "images/%s/%s_%s.jpg" % (opt.outf, opt.outf, batches_done), normalize=False)
            ...
            for i, batch in enumerate(self.dataloader):
                ...
                if opt.loss_mode == 'mask':
                    ...
                    if i%10==0:
                        loss_mask_ = 0
                        print(fake_B[0])
                        SaveFirstImage(fake_B.mul(A_mask), 'mask.png')
                        SaveFirstImage(white.mul(A_mask), 'white.png')
                        SaveFirstImage(fake_B, 'fake.png')
                    self.loss_mask_ = (self.loss_mask_A + self.loss_mask_B)/2
                    
                # perceptual loss
                
                '''END'''
                # Cycle loss
                recov_A = self.G_AB(fake_B)
                recov_B = self.G_BA(fake_A)
                ...
                # Total loss
                self.loss_G = self.loss_GAN + opt.lambda_cyc * self.loss_cycle + opt.lambda_id * self.loss_identity + opt.lambda_mask * self.loss_mask_\
                        + opt.lambda_perc * self.loss_perc
                ...
                # Print log
                sys.stdout.write(
                    "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [G loss: %f, adv: %f, cycle: %f, identity: %f, mask: %f] ETA: %s"
                    % (
                        epoch,
                        opt.n_epochs,
                        i,
